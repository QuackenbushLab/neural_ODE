...
from sympy import Symbol, sympify, sin, cos
...
def decode_chromosome(individual, const_reg, num_var_reg, data, data_dim, dim):
    ...
    decoded_val[:,0] = reg_mat[:,dim]

    return decoded_val

def evaluate_individual(decoded_chromosome, target, dim):
    ''' Currently using MSE for evaluation '''
    mse = np.mean(np.square(decoded_chromosome - target[:,dim]))
    return 1/mse
...
def crossover(chrom1, chrom2, max_instructions):
    ...
    updated_chrom1 = chrom1[0:chrom1_crosspt1-1]
    updated_chrom1 = updated_chrom1 + chrom2[chrom2_crosspt1-1:chrom2_crosspt2-1] + chrom1[chrom1_crosspt2-1::]

    updated_chrom2 = chrom2[0:chrom2_crosspt1-1]
    updated_chrom2 = updated_chrom2 + chrom1[chrom1_crosspt1-1:chrom1_crosspt2-1] + chrom2[chrom2_crosspt2-1::]
...
def insert_best_individual(population, best_chromosome, num_copies_to_insert):
    for i in range(num_copies_to_insert):
        population[i] = best_chromosome.copy()
    return population
...
def get_symbolic_expression(chromosome, const_reg, num_var_reg, data_dim, dim):
    ...
    combined_reg = [0 for x in range(num_var_reg)] + const_reg + [0 for x in range(data_dim)]
    for i in range(data_dim):
        combined_reg[i] = Symbol('x{}'.format(i))
        combined_reg[-i - 1] = Symbol('x{}'.format(i))
    ...
    simplified_eq = sympify(combined_reg[0])
        #simplified_eq[i] = simplified_eq[i].replace('**', '^')
    return simplified_eq

parser = argparse.ArgumentParser('Testing')
parser.add_argument('--data', type=str, default='.\\data\\2d_parabolic_drag_random_x_and_y.csv')
parser.add_argument('--model', type=str, default='D:\\Skola\MSc-Thesis\\fully_trained\\2019-3-19(14;11)_2d_parabolic_drag_random_x_and_y_40epochs\\best_model.pt')
args = parser.parse_args()

if __name__ == "__main__":
    print("LGP for ODENet function fitting v2.0")
    ...
    int_reg_range = 10
    num_decimals = 3
    const_reg = [np.around(np.random.rand()*0.1, decimals=num_decimals)
                ] + [np.around(np.random.rand(), decimals=num_decimals) for x in range(num_float_reg)
    ...
    for curr_dim in range(data_dim):
        ...
        while error > target_error:
            ...
            for i in range(pop_size):
                ...
                # Decode individual
                #print("Decode")
                decoded_chromosome = decode_chromosome(individual, const_reg, num_var_reg, data, data_dim, curr_dim)
            ...
            best_chromosome = population[best_indx]
    ...
    for i in range(len(each_dim_output)):
        print(colored("\tx{}_dot = {}".format(i, each_dim_output[i]), 'green'))
