...
def get_true_val_velocities(odenet, data_handler, method, batch_type):
    ...
    #velocities = torch.zeros(data_pw.shape).to(data_handler.device)
    pred_vels = odenet.prior_only_forward(t_pw,data_pw)
    ...
    return {'x': data_pw, 'velo_x': pred_vels} 
...
if __name__ == "__main__":
    ...
    X = get_true_val_velocities(odenet, data_handler, settings['method'], settings['batch_type'])
    print(X['x'].shape)
    print(X['velo_x'].shape)
