...
for(this_gene in all_genes){
  this_gene_file <- paste0("C:/STUDIES/RESEARCH/neural_ODE/all_manuscript_models/breast_cancer/go_biol_enrichment_full_set_genome_background/enrichment_",
                           this_gene,".csv")
  ...
  all_top_paths <- c(all_top_paths,
                     trimws(D[nGenes >= ng_min,][order(-nGenes/Pathway_Genes),][1:num_tops, Pathway]))
...
for(this_gene in all_genes){
  ...
  D <- fread(this_gene_file)
  names(D) <- gsub(" ","_", names(D))
  D <-  D[order(-nGenes/Pathway_Genes),]
  D_to_take <- D[trimws(Pathway) %in% all_top_paths, 
                 .(pathway = trimws(Pathway), fold_enrich = nGenes/Pathway_Genes)]
...
write.csv(all_enrich_wide,
          "C:/STUDIES/RESEARCH/neural_ODE/all_manuscript_models/breast_cancer/all_enrichments_go_biol_full_wide.csv",
          row.names = F,
          na = "")
